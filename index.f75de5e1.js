...
function valuesValidation(inputToCheck) {
    ...
    if (position === "" || office === "" || salary === "") return {
        type: "error",
        text: ERROR_EMPTY_FIELDS
    };
    if (nameValue.length < 4) return {
        type: "error",
        text: ERROR_TO_SHORT_NAME
    };
    if (age < 18) return {
        type: "error",
        text: ERROR_TO_SMALL_AGE
    };
    if (age > 90) return {
        type: "error",
        text: ERROR_TO_BIG_AGE
    };
    return {
        type: "success",
        text: SUCCESSFUL_INPUT
    };
}
// #endregion
...
function cellValid(input) {
    ...
    else if (currentIndex === 3 && +value < 18 || value.trim() === "") return {
        type: "warning",
        text: WARNING_SMALL_AGE
    };
    else if (currentIndex === 3 && +value > 90 || value.trim() === "") return {
        type: "warning",
        text: WARNING_BIG_AGE
    };
    return {
        type: "success",
        text: SUCCESS_VALIDATION
    };
} // #endregion
